...
{
    public class AsmBuiltInTypeProvider : IBuiltInTypeProvider
    {
        private readonly Assembly[] dependentAssemblies;

        public AsmBuiltInTypeProvider(Assembly[] dependentAssemblies)
        {
        ...
        public (IType[] delegateConstructorTypes, IType voidType, IConstructorInfo objectConstructorInfo) GenerateAssemblyTypes(CodeGenerationStore store)
        {
            ...
            {
                var types = assembly.GetTypes().Where(x => x.IsPublic).Select(x => new AsmType(x));
            ...
            store.Types.Clear();
            foreach (var type in AsmType.TypeCache)
            {
